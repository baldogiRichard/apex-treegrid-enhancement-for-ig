...
(function($) {
	...
	var methods = {
		option: function(optionName, value) {
			var settings = this.data('apex-treegrid-settings');
			...
			if ($.isPlainObject(optionName)) {
				...
				return this.data('apex-treegrid-settings', settings);
			}
		...
		getDepth: function() {
			return this.data('apex-treegrid-depth');
		},
		toggle: function() {
		...
		remove: function() {
			...
			this.each(function() {
				...
				_initNode($parent, $parent.data('apex-treegrid-depth'));
			});
		...
		move: function(target, position) {
			...
			this.removeClass('apex-treegrid-parent-' + _getParentId(this));
			if (id !== null) this.addClass('apex-treegrid-parent-' + id);
			...
			//reinit
			if ($oldParent.length) _initNode($oldParent, $oldParent.data('apex-treegrid-depth'));
			var $parent = _getParent(this);
			if ($parent.length) _initNode($parent, $parent.data('apex-treegrid-depth'));
			else _initNode(this);
		},
		getRoots: function() {
			var items = [];
	...
	function _initTree(options) {
		...
		return $this.each(function() {
			...
			//settings
			$this.data('apex-treegrid-settings', settings);

			//event handlers
			$this
				.on('click', '>tbody>tr>td:first-child>.apex-treegrid-container>.apex-treegrid-expander', _expanderClick)
				.on('mousedown', '>tbody>tr>td:first-child>.apex-treegrid-container', _nodeMouseDown);
	...
	function _expand($this) {
		//event callback
		var settings = $this.closest('table').data('apex-treegrid-settings');
		if (!settings.onExpand.call($this)) return;
		//node class
		$this.addClass('expanded');
		//expander class
		$this.find('>td:first>.apex-treegrid-container>.apex-treegrid-expander')
			.removeClass('apex-treegrid-expander-collapsed')
	...
	function _collapse($this) {
		...
		//expander class
		$this.find('>td:first>.apex-treegrid-container>.apex-treegrid-expander')
			.removeClass('apex-treegrid-expander-expanded')
	...
	function _add($this, items) {
		...
		$.each(items, function(i, item) {
			var $tr = $(item);
			//if already exists
			if ($table.find('>tbody>tr.apex-treegrid-' + _getId($tr)).length) return;
			//parent id
			if (parentId !== null) $tr.addClass('apex-treegrid-parent-' + parentId);
			//cell count
		...
		_initNode($this, $this.data('apex-treegrid-depth'));
		//callbask
		var settings = $this.closest('table').data('apex-treegrid-settings');
		settings.onAdd.call($this, childs);
	};
	function _getParent($this) {
		return $this.parent().find('>.apex-treegrid-' + _getParentId($this));
	};
	function _getChildNodes($this) {
		return $this.parent().find('>.apex-treegrid-parent-' + _getId($this));
	};
	...
	function _getId($this) {
		var template = /apex-treegrid-([A-Fa-f0-9_]+)/;
	...
	function _getParentId($this) {
		var template = /apex-treegrid-parent-([A-Fa-f0-9_]+)/;
		if (template.test($this.attr('class'))) {
			return template.exec($this.attr('class'))[1];
		};
		return null;
	...
	function _initNode($this, depth, forceExpand) {
		...
		$this.each(function() {
			var $this = $(this).data('apex-treegrid-depth', depth);
			//child nodes
			...
			//container
			$td = $this.find('>td:first'), $container = $td.find('>.apex-treegrid-container');
			if ($container.length === 0) {
				$container = $('<div class="apex-treegrid-container">').html($td.html());
				$td.html('').append($container);
			};
			//expander
			$container.find('.apex-treegrid-expander').remove();
			$expander = $('<span class="apex-treegrid-expander">').prependTo($container);
			if ($this.data('count')) {
				if ($this.hasClass('expanded')) $expander.addClass('apex-treegrid-expander-expanded');
				else $expander.addClass('apex-treegrid-expander-collapsed');
			};
			//indent
	...
	function _expanderClick() {
		var $this = $(this);
		if ($this.hasClass('apex-treegrid-expander-expanded') || $this.hasClass('apex-treegrid-expander-collapsed')) {
	...
	function _nodeMouseDown(e) {
		...
		//move enabled?
		var $this = $(this), settings = $this.closest('table').data('apex-treegrid-settings');
		if (!settings.enableMove) return;
		//node expander?
		var $el = $(e.target);
		if ($el.hasClass('apex-treegrid-expander')) return;
		//handle
	...
	function _nodeMouseMove(e) {
		var d = Math.max(Math.abs(e.pageX - downX), Math.abs(e.pageY - downY)),
			settings = $moveItem.closest('table').data('apex-treegrid-settings');
	...
	function _dragStart(e) {
		...
		_makeDropMap();
		//make helper
		$moveHelper = $moveItem.find('>td:first>.apex-treegrid-container').clone().addClass('dragging').css({
			left: e.pageX + offX,
			top: e.pageY + offY
		});
		$moveHelper.find('>.apex-treegrid-expander').remove();
		$moveHelper.appendTo('body');
		//make indicator
		if ($('.apex-treegrid-move-indicator').length) return;
		$indicator = $('<div class="apex-treegrid-move-indicator">').appendTo('body');
		//event callback
		var $treegrid = $moveItem.closest('table'), settings = $treegrid.data('apex-treegrid-settings');
	...
	function _dragOver(info) {
		...
		if (isTarget) $indicator.css({
			'display': 'block',
			'left': info.node.find('>td:first>.apex-treegrid-container').offset().left,
			'top': info.top
		});
	};
	function _dragOut() {
